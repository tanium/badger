...
// TestVerifyVPReset verifies entry value pointer flags are reset
...
// To see the slice error prior to `db` fix replace db.writeToLSM line 687:
// 			Meta:      entry.meta &^ bitValuePointer,
// with:
// 			Meta: entry.meta,
func TestVerifyVPReset(t *testing.T) {
	dir, err := ioutil.TempDir("", "badger-test")
	require.NoError(t, err)
	defer removeDir(dir)
